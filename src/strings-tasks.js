...
function countVowels(str) {
  return str.split('').reduce((acc, curr) => {
    if (['a', 'e', 'i', 'o', 'u', 'y'].includes(curr.toLowerCase())) {
      return acc + 1;
    }
    return acc;
  }, 0);
...
function findLongestWord(sentence) {
  ...
  return arr.reduce((largest, curr) => {
    return curr.length > largest.length ? curr : largest;
  }, arr[0]);
}
...
function invertCase(str) {
  return str
    .split('')
    .map((w) => (w === w.toUpperCase() ? w.toLowerCase() : w.toUpperCase()))
    .join('');
}
...
function encodeToRot13(str) {
  ...
  for (let i = 0; i < str.length; i += 1) {
    res += cipheredAlphabet[originalAlphabet.indexOf(str[i])] || str[i];
  }
  return res;
}
